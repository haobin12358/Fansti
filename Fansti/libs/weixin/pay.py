...
class WeixinPay(object):
    ...
    def __init__(self, app_id, mch_id, mch_key, notify_url=None, key=None, cert=None):
        self.app_id = app_id
        self.mch_id = mch_id
    ...
    def _fetch_chengxu(self, url, data, use_cert=False):
    ...
    def pay_individual(self, **data):
        ...
        if "openid" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数openid")
        if "amount" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数amount")
        if "desc" not in data:
            raise WeixinPayError("退款申请接口中，缺少必填参数desc")
        data.setdefault('check_name', 'NO_CHECK')
        return self._fetch_chengxu(url, data, True)
