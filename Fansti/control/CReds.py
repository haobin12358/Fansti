...
class CReds():
    ...
    def get_all_red(self):
        ...
        all_red = get_model_return_list(self.sred.get_all_red())
        make_log("all_red", all_red)
        response = import_status("SUCCESS_GET_RED", "OK")
        response["data"] = {}
        index = len(all_red)
        while index > 0:
            red_id = self.sred.get_id_by_red_loginname(all_red[index - 1]["id"], args["login_name"])
            make_log("red_id", red_id)
            if red_id:
                all_red.remove(all_red[index - 1])
            index = index - 1
        response["data"]["red_num"] = len(all_red)
        response["data"]["red_list"] = all_red
        my_red = get_model_return_list(self.sred.get_my_red(args["login_name"]))
        ...
        while month < 12:
            my_red_month = get_model_return_list(self.sred.get_my_red_by_time(
                ...
                datetime.datetime(datetime.datetime.now().year, month, calendar.monthrange(
                    datetime.datetime.now().year, month), 23, 59, 59)
            ))
            month_red = {
                "month": month,
