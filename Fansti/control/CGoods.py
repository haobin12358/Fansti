...
class CGoods():
    ...
    def get_goods_list(self):
        ...
        goods_list = get_model_return_list(self.sgoods.get_all_goods_by_user(args["login_name"]))
    ...
    def get_jc_abo(self):
        ...
        jc_abo = get_model_return_dict(self.sgoods.get_jd_by_jcno(args["jcno"]))
        ...
        if jc_abo_in:
            for in_order in jc_abo_in:
                jc_abo["in"]["createtime"] = in_order["createtime"]
        ...
        if jc_abo_out:
            for out_order in jc_abo_out:
                jc_abo["out"]["createtime"] = out_order["createtime"]
        ...
        if jc_abo_weight:
            for weight_order in jc_abo_weight:
                jc_abo["weight"]["createtime"] = weight_order["createtime"]
